    ...
    public:
        ...
        std::pair<double, double> calculer_force(const Voiture& voiture) const override {
            ...
            if (voiture.isFreinActif()) {
                Fmax *= 0.3; // Perte de grip sous freinage
            }
...
class ForceAerodynamique : public Force {
    ...
    // Frein à main (glissement)
    class ForceFreinGlisse : public Force {
        private:
            double intensite;
        public:
            ForceFreinGlisse(double intensite) : intensite(intensite) {}
        
            std::pair<double, double> calculer_force(const Voiture& v) const override {
                double vx = v.getVx(), vy = v.getVy();
                double v_module = std::sqrt(vx * vx + vy * vy);
                if (v_module < 0.01) return {0, 0};
        
                return { -intensite * (vx / v_module), -intensite * (vy / v_module) };
            }
        };
